package com.globallogic.webapp.spring;
...
/**
 * <p>Adds commons validator configuration files to an existing Spring commons
 ...
 *
 * <pre>
 * &lt;bean class=&quot;com.globallogic.webapp.spring.ValidatorExtensionPostProcessor&quot;&gt;
 ...
 * </pre>
 *
 * <p>The sample adds a single validation configuration file
 * (foo-validation.xml) to an existing Spring commons Validator Factory bean (a
 * bean of class org.springmodules.validation.commons.DefaultValidatorFactory).
 * Assuming the validator extension is included in a Spring configuration file
 * called applicationContext-foo-validation.xml, and that this configuration
 * file is added directly below WEB-INF in the Foo web project, then the normal
 * war overlay process coupled with AppFuse's configuration file auto detection
 * will ensure that the validation extension is automatically included into the
...
public class ValidatorExtensionPostProcessor implements BeanFactoryPostProcessor {
     ...
     * factory.
     *
     * @param validationConfigLocations The list of additional validation
